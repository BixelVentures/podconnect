...
Each HomePod is its **own independent Connect speaker**. With **one Spotify account** you play to
**one room at a time** (switch in the Connect picker). **Different music in different rooms at the
same time** needs **multiple Spotify accounts** (one per room) — see
[`MULTI-ACCOUNT.md`](MULTI-ACCOUNT.md). Synchronized *same* music across rooms (groups) is a separate,
