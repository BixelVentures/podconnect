...
| R5 | **"Play X" via Gemini plays the wrong thing / blasts on all speakers** | PodVoice calls `POST /api/play?query=<x>`; `/api/play` **ignored `query`** and sent plain **resume** to **all rooms**. **RESOLVED**: (a) Speakers 0.19.0 rejects `?query=` (400); (b) PodVoice 0.11.0 went fully generic (music via HA `media_player` on the Control entity, no Control-specific code); (c) **Control 0.8.0** makes `media_player.play_media` accept a free-text name → searches + plays the top result, so "play X" works in one standard generic call | `media_player.async_play_media` + `_search_top_uri`; `/api/play` `main.go:1132+` | ✅ **resolved — Control 0.8.0 + PodVoice 0.11.0** |
