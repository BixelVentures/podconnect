...
| R5 | **"Play X" via Gemini plays the wrong thing / blasts on all speakers** | PodVoice calls `POST /api/play?query=<x>` on PodConnect; `/api/play` **ignores `query`** and sends plain **resume** to **all rooms** (`targetRooms` defaults to all). go-librespot is a Connect *receiver* — it can't search/start a track. "Play X" must go via the **Web API / Control** (`media_player.play_media`), not PodConnect | `/api/play` `main.go:1132-1142`, `targetRooms` `main.go:1274-1283` | 🔴 wrong layer (cross-add-on) |
